...
export class UiScene extends Phaser.Scene {
  ...
  public async updateHealthInHud(data: PlayerHealthUpdated): Promise<void> {
    ...
    for (let i = 0; i < healthDifference; i += 1) {
      const hearthIndex = Math.round(health / 2) - 1;
      const isHalfHearth = health % 2 == 1;
      let animationName = HEART_ANIMATIONS.LOSE_LAST_HALF;
      if (!isHalfHearth) {
        animationName = HEART_ANIMATIONS.LOSE_FIRST_HALF;
      }
      await new Promise((resolve) => {
        this.#hearts[hearthIndex].play(animationName);
        this.#hearts[hearthIndex].once(Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + animationName, () => {
          resolve(undefined);
        });
      });
      health -= 1;
