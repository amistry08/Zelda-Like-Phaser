...
import {
  ENEMY_BOSS_TELPORT_STATE_FINISHED_DELAY,
  ENEMY_BOSS_TELPORT_STATE_INITIAL_DELAY,
} from '../../../../../../common/config';
...
export class BossDrowTeleportState extends BaseCharacterState {
  ...
  onEnter(): void {
    ...
    const timeEvent = this._gameObject.scene.time.addEvent({
      delay: ENEMY_BOSS_TELPORT_STATE_INITIAL_DELAY,
      callback: () => {
  ...
  #handleTeleportFinished(): void {
    ...
    this._gameObject.scene.time.delayedCall(ENEMY_BOSS_TELPORT_STATE_FINISHED_DELAY, () => {
